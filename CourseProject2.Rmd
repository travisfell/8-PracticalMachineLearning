...
test <- select(test, contains("accel"))
...
# create new classe2 column to store numeric values equivalent to the original classe varible
train$classe2 <- NA
...
mrm <- train(classe ~ ., method = "lm", data = train)
mrm$finalModel
predTest <- predict(mrm, test)
summary(predTest)
